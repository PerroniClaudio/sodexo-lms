...
class DeleteCourseClassTutorsRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'assignment_ids.*' => ['integer', 'distinct', 'exists:course_class_tutors,id'],
        ];
    }
    ...
    public function after(): array
    {
        return [
            function (Validator $validator): void {
                ...
                $matchingAssignmentsCount = CourseClassTutor::query()
                    ->where('course_class_id', $courseClass->getKey())
                    ->whereKey($assignmentIds)
                    ->count();

                if ($matchingAssignmentsCount !== count($assignmentIds)) {
                    $validator->errors()->add('assignment_ids', __('Una o più assegnazioni tutor non appartengono a questa classe.'));
                }
            },
        ];
    }
