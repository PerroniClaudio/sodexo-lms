...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('job_based_requirement_user_certificate', function (Blueprint $table) {
            $table->foreignId('user_certificate_id')
                ->constrained(indexName: 'jbr_user_certificate_certificate_fk')
                ->cascadeOnDelete();
            $table->foreignId('job_based_requirement_id')
                ->constrained(indexName: 'jbr_user_certificate_requirement_fk')
                ->cascadeOnDelete();
