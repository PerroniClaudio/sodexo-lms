<div id="quiz-validity-badge" class="flex items-center gap-3">
    ...
    <span data-invalid-reason class="text-xs text-error" style="display: {{ $module->isValidQuiz() ? 'none' : 'inline' }};">{{ __('Deve avere almeno una domanda valida, il punteggio di superamento non può essere più alto del punteggio massimo e i tentativi devono essere maggiori di zero.') }}</span>
</div>