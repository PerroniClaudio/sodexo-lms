<div id="module-validity-badge" class="flex items-center gap-3" data-validity-details data-validity-modal-target="#module-validity-details-modal">
    <span data-valid-badge class="badge badge-sm badge-success" style="display: {{ $isValid ? 'inline-flex' : 'none' }};">{{ __('Valido') }}</span>
    <button type="button" data-invalid-badge data-open-validity-details-modal class="badge badge-sm badge-error whitespace-nowrap cursor-pointer" style="display: {{ $isValid ? 'none' : 'inline-flex' }};">{{ __('Non valido') }}</button>
</div>
