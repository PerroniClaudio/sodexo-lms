<x-layouts.admin>
    ...
    <div class="mx-auto flex w-full max-w-7xl flex-col gap-6 p-4 sm:p-6 lg:p-8">
        ...
        >
            <x-slot:filters>
                <form method="GET" action="{{ route('admin.job-categories.index') }}" class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex w-full max-w-md items-center gap-2">
                        ...
                        <label class="input input-bordered flex w-full items-center gap-2">
                            ...
                            <input
                                ...
                                value="{{ $tableSearch }}"
                                class="grow"
                                placeholder="{{ __('Cerca nelle categorie') }}"
                            >
                        </label>
                ...
                <tr class="hover:bg-base-200">
                    ...
                    <td>
                        <div class="flex gap-2">
                            @if(!$category->trashed())
                                <a href="{{ route('admin.job-categories.edit', $category) }}" class="btn btn-primary btn-sm">
                                    {{ __('Modifica') }}
                                </a>
                                <form method="POST" action="{{ route('admin.job-categories.destroy', $category) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-error btn-sm" 
                                            onclick="return confirm('{{ __('Sei sicuro di voler eliminare questa categoria?') }}')">
                                        {{ __('Elimina') }}
                                    </button>
                                </form>
                            @else
