<x-layouts.admin>
    <div class="mx-auto flex w-full max-w-7xl flex-col gap-6 p-4 sm:p-6 lg:p-8">
        <x-page-header :title="__('Modifica mansione')">
            <x-slot:actions>
                <form method="POST" action="{{ route('admin.job-titles.destroy', $title) }}" onsubmit="return confirm('{{ __('Confirm deletion') }}')">
                    @csrf
                    @method('DELETE')
                    ...
                    <div class="form-control">
                        <label class="label cursor-pointer justify-start gap-3">
                            <input
                                ...
                                value="1"
                                class="checkbox"
                                @checked(old('is_active', $title->is_active))
                            >
