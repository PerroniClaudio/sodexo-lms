<x-layouts.admin>
    <div class="mx-auto flex w-full max-w-5xl flex-col gap-6 p-4 sm:p-6 lg:p-8">
        <x-page-header :title="__('Associazione utenti corsi')">
            <x-slot:actions>
                <a href="{{ route('admin.imports.user-courses.template') }}" class="btn btn-outline">
        ...
        <div class="card border border-base-300 bg-base-100 shadow-sm">
            <div class="card-body gap-6">
                ...
                <form method="POST" action="{{ route('admin.imports.user-courses.store') }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    ...
                    <div class="rounded-box border border-base-300 bg-base-200/40 p-4 text-sm text-base-content/80">
                        ...
                        <ul class="mt-3 list-disc space-y-2 pl-5">
                            ...
                            <li>{{ __('Seconda colonna obbligatoria: codice corso esistente e pubblicato.') }}</li>
                            <li>{{ __('Se l\'utente è già iscritto attivamente, l\'import aggiorna l\'origine diretta senza creare duplicati.') }}</li>
                            <li>{{ __('Se esiste un\'iscrizione eliminata, l\'import la ripristina solo se i destinatari del corso lo consentono.') }}</li>
                            <li>{{ __('Le nuove iscrizioni passano dagli stessi controlli del flusso admin singolo: prerequisiti del corso e destinatari.') }}</li>
                        </ul>
                    </div>
        ...
        <div
            data-user-course-import-status-card
            data-status-url="{{ route('admin.imports.user-courses.status-card') }}"
        >
            <x-admin.imports.user-courses-status-card :data="array_merge(get_defined_vars(), ['recentImports' => $recentImports])" />
        </div>
    </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const statusCard = document.querySelector('[data-user-course-import-status-card]');
