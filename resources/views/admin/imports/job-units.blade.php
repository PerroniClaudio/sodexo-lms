<x-layouts.admin>
    <div class="mx-auto flex w-full max-w-5xl flex-col gap-6 p-4 sm:p-6 lg:p-8">
        <x-page-header :title="__('Import unità lavorative')">
            <x-slot:actions>
                <a href="{{ route('admin.imports.job-units.template') }}" class="btn btn-outline">
        ...
        <div class="card border border-base-300 bg-base-100 shadow-sm">
            <div class="card-body gap-6">
                ...
                <form method="POST" action="{{ route('admin.imports.job-units.store') }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    ...
                    <div class="rounded-box border border-base-300 bg-base-200/40 p-4 text-sm text-base-content/80">
                        ...
                        <ul class="mt-3 list-disc space-y-2 pl-5">
                            <li>{{ __('Obbligatori sempre: codice unità lavorativa, nome, paese, regione, città, codice postale.') }}</li>
                            <li>{{ __('Paese atteso: IT.') }}</li>
                            <li>{{ __('Regione, provincia e città devono esistere nell’archivio geografico.') }}</li>
                            <li>{{ __('Se il codice unità lavorativa esiste già, record viene aggiornato.') }}</li>
                        </ul>
                    </div>
        ...
        <div
            data-job-unit-import-status-card
            data-status-url="{{ route('admin.imports.job-units.status-card') }}"
        >
            <x-admin.imports.job-units-status-card :data="array_merge(get_defined_vars(), ['recentImports' => $recentImports])" />
        </div>
    </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const statusCard = document.querySelector('[data-job-unit-import-status-card]');
