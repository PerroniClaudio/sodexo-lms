<x-layouts.admin>
    <div class="mx-auto flex w-full max-w-5xl flex-col gap-6 p-4 sm:p-6 lg:p-8">
        <x-page-header :title="__('Associazione mansioni rischio')">
            <x-slot:actions>
                <a href="{{ route('admin.imports.job-task-risk-associations.template') }}" class="btn btn-outline">
        ...
        <div class="card border border-base-300 bg-base-100 shadow-sm">
            <div class="card-body gap-6">
                ...
                <form method="POST" action="{{ route('admin.imports.job-task-risk-associations.store') }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    ...
                    <div class="rounded-box border border-base-300 bg-base-200/40 p-4 text-sm text-base-content/80">
                        ...
                        <ul class="mt-3 list-disc space-y-2 pl-5">
                            <li>{{ __('Obbligatori sempre: codice mansione, nome settore, livello di rischio, sovrascrivi rischio settore.') }}</li>
                            <li>{{ __('Codice mansione e nome settore devono esistere.') }}</li>
                            <li>{{ __('Livello di rischio ammesso: basso, medio, alto.') }}</li>
                        </ul>
                    </div>
        ...
        <div
            data-job-task-risk-association-import-status-card
            data-status-url="{{ route('admin.imports.job-task-risk-associations.status-card') }}"
        >
            <x-admin.imports.job-task-risk-associations-status-card :data="array_merge(get_defined_vars(), ['recentImports' => $recentImports])" />
        </div>
    </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const statusCard = document.querySelector('[data-job-task-risk-association-import-status-card]');
