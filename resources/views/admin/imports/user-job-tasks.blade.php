<x-layouts.admin>
    <div class="mx-auto flex w-full max-w-5xl flex-col gap-6 p-4 sm:p-6 lg:p-8">
        <x-page-header :title="__('Associazione utenti mansioni')">
            <x-slot:actions>
                <a href="{{ route('admin.imports.user-job-tasks.template') }}" class="btn btn-outline">
        ...
        <div class="card border border-base-300 bg-base-100 shadow-sm">
            <div class="card-body gap-6">
                ...
                <form method="POST" action="{{ route('admin.imports.user-job-tasks.store') }}" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    ...
                    <div class="rounded-box border border-base-300 bg-base-200/40 p-4 text-sm text-base-content/80">
                        ...
                        <ul class="mt-3 list-disc space-y-2 pl-5">
                            ...
                            <li>{{ __('Ogni altra colonna rappresenta una mansione esistente, identificata da codice.') }}</li>
                            <li>{{ __('Valore accettato per associare: SI.') }}</li>
                            <li>{{ __('Import aggiunge solo mansioni mancanti; non rimuove quelle già presenti.') }}</li>
                        </ul>
                    </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const statusCard = document.querySelector('[data-user-job-task-import-status-card]');
