<x-layouts.admin>
    ...
    <div class="mx-auto flex w-full max-w-7xl flex-col gap-6 p-4 sm:p-6 lg:p-8">
        ...
        >
            <x-slot:filters>
                <form method="GET" action="{{ route('admin.job-levels.index') }}" class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div class="flex w-full max-w-md items-center gap-2">
                        ...
                        <label class="input input-bordered flex w-full items-center gap-2">
                            ...
                            <input
                                ...
                                value="{{ $tableSearch }}"
                                class="grow"
                                placeholder="{{ __('Cerca nei livelli') }}"
                            >
                        </label>
                ...
                <tr class="hover:bg-base-200">
                    ...
                    <td>
                        <div class="flex gap-2">
                            @if(!$level->trashed())
                                <a href="{{ route('admin.job-levels.edit', $level) }}" class="btn btn-primary btn-sm">
                                    {{ __('Modifica') }}
                                </a>
                                <form method="POST" action="{{ route('admin.job-levels.destroy', $level) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-error btn-sm" 
                                            onclick="return confirm('{{ __('Sei sicuro di voler eliminare questo livello?') }}')">
                                        {{ __('Elimina') }}
                                    </button>
                                </form>
                            @else
                                <form method="POST" action="{{ route('admin.job-levels.restore', $level->id) }}" class="inline">
                                    @csrf
